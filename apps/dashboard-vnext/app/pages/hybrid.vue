...
import { AlertTriangle, ArrowRight, Box, Check, CheckCircle2, ChevronRight, CircleHelp, Clock3, Code2, Columns3, Copy, FileCheck2, FileInput, FileText, FolderPlus, GitBranch, GripVertical, LayoutList, Link2, Minus, Play, Plus, Search, ShieldCheck, TerminalSquare, Trash2, UserCheck, Pencil, Save, X } from 'lucide-vue-next'
import type { DashboardState, EngineReadinessState, ExecutionTemplateRenderResult, LaunchMode, LaunchPlatform, LaunchRequestRecord, OperationKind, PipelineReviewOverview, ProjectCatalogState, ProjectSkillManagementState, ProjectSkillStatus, RunDisplayStatus, RunReviewStatus, SessionSortMode, TrustedAutoStatus, WorkflowRun, WorkSession } from '../../shared/types/dashboard'
import ExecutionTemplateDialog from '~/components/ExecutionTemplateDialog.vue'
import { searchProject } from '~/utils/dashboardSearch'
import { buildRunChoices } from '~/utils/runChoices'
import { runSelectionFor } from '~/utils/runSelection'
import { needsReview, needsUserReview } from '~/utils/runReviewState'
...
const pendingReviewRuns = computed(() => passedRuns.value.filter(needsUserReview))
const attentionRuns = computed(() => activeRuns.value.filter(run => run.status !== 'pass'))
const reviewQueueRuns = computed(() => activeRuns.value.filter(needsReview))
const reviewCount = computed(() => reviewQueueRuns.value.length)
...
const pipelineReviewOpen = ref(false)
const pipelineReviewBusy = ref(false)
const pipelineReviewError = ref('')
const pipelineReviewOverview = ref<PipelineReviewOverview | null>(null)
const executionTemplateOpen = ref(false)
const sessionContextMenu = ref<{ session: WorkSession; x: number; y: number } | null>(null)
const sessionRemoving = ref(false)
const searchQuery = ref('')
const searchResults = computed(() => searchProject(project.value, searchQuery.value))
const assetPanelKind = ref<'evidence' | 'artifact' | null>(null)
...
function closeSessionContextMenuOnEscape(event: KeyboardEvent) {
...
onMounted(() => {
  ...
  window.addEventListener('keydown', closeSessionContextMenuOnEscape)
})
onBeforeUnmount(() => {
  if (pendingLaunchTimer) clearInterval(pendingLaunchTimer)
  window.removeEventListener('click', closeSessionContextMenu)
  window.removeEventListener('keydown', closeSessionContextMenuOnEscape)
})
...
function openSessionContextMenu(event: MouseEvent, session: WorkSession) {
  if (source.value?.mode !== 'live') return
...
function openTemplateFromContext() {
  closeSessionContextMenu()
  executionTemplateOpen.value = true
}

function openEmptySessionFromContext() {
  closeSessionContextMenu()
  openSessionEditor()
}

async function removeSessionFromList(target: WorkSession) {
  if (!target || !project.value?.source_root || sessionRemoving.value) return
  const runNotice = target.runs.length
    ? `연결된 Run ${target.runs.length}개는 원본과 산출물을 보존한 채 대시보드 목록에서 숨겨집니다.`
    : '실제 프로젝트 폴더와 산출물은 삭제되지 않습니다.'
  const confirmed = window.confirm(`'${target.name}' 작업 세션을 목록에서 제거할까요?\n\n${runNotice}`)
  if (!confirmed) return
  sessionRemoving.value = true
  ...
  try {
    await $fetch('/api/work-sessions', {
      ...
      body: {
        ...
        run_ids: target.runs.map(run => run.run_id),
      },
    })
    const removedSelectedSession = selectedSessionId.value === target.session_id
    closeSessionContextMenu()
    await refresh()
    if (removedSelectedSession) {
      selectedSessionId.value = sessions.value[0]?.session_id ?? ''
      selectedRunId.value = sessions.value[0]?.runs[0]?.run_id ?? ''
    }
  } catch (error: any) {
    sessionOrderError.value = error?.data?.statusMessage ?? '작업 세션을 제거하지 못했습니다.'
  } finally {
    sessionRemoving.value = false
  }
}

async function removeContextSession() {
  const target = sessionContextMenu.value?.session
  if (!target) return
  await removeSessionFromList(target)
}
...
async function acceptExecutionTemplate(result: ExecutionTemplateRenderResult) {
  if (!result.session_id) return
  selectedSessionId.value = result.session_id
  selectedRunId.value = ''
  await refresh()
  executionTemplateOpen.value = false
}
...
function chooseRun(run?: WorkflowRun) {
  if (!run) return
  const selection = runSelectionFor(sessions.value, run.run_id)
  if (!selection) return
  selectedSessionId.value = selection.session_id
  selectedRunId.value = selection.run_id
}
function navigateSummary(target: 'pass' | 'review' | 'conflict') {
  if (target === 'pass') {
    if (passRun.value) chooseRun(passRun.value)
    viewMode.value = 'board'
    return
  }
  if (target === 'review') {
    if (reviewQueueRuns.value[0]) chooseRun(reviewQueueRuns.value[0])
    viewMode.value = 'focus'
    return
  }
  if (relationSessions.value[0]) chooseSession(relationSessions.value[0])
  viewMode.value = 'board'
}
function chooseSearchResult(result: { session_id: string; run_id?: string }) {
  const session = sessions.value.find(item => item.session_id === result.session_id)
  if (!session) return
  selectedSessionId.value = session.session_id
  selectedRunId.value = result.run_id ?? session.runs[0]?.run_id ?? ''
  viewMode.value = 'focus'
  searchQuery.value = ''
}
function openAssetPanel(kind: 'evidence' | 'artifact') {
  if (!selectedRun.value) return
  assetPanelKind.value = kind
}
function openReviewDialog(run: WorkflowRun | undefined = selectedRun.value) {
  if (!run) return
  chooseRun(run)
  reviewError.value = ''
  reviewDialogOpen.value = true
}
async function saveRunReview(payload: { status: RunReviewStatus; note: string }) {
  if (!selectedRun.value || reviewSaving.value) return
  ...
  try {
    await $fetch('/api/run-metadata', {
      ...
      body: {
        run_id: selectedRun.value.run_id,
        display_title: selectedRun.value.display_title ?? '',
        user_note: selectedRun.value.user_note ?? '',
        tags: selectedRun.value.tags ?? [],
        display_status: selectedRun.value.display_status ?? 'active',
    ...
    reviewError.value = '검토 기록을 저장하지 못했습니다. 입력 내용과 저장 경로를 확인해 주세요.'
  } finally {
...
function reviewLabel(run?: WorkflowRun) {
  const status = run?.review_status ?? 'unreviewed'
  return status === 'approved' ? '검토 승인' : status === 'changes_requested' ? '수정 필요' : status === 'deferred' ? '검토 보류' : '검토 대기'
}
...
async function refreshPipelineReview() {
  if (!project.value?.source_root || !selectedSession.value || source.value?.mode !== 'live') return
  pipelineReviewBusy.value = true
  pipelineReviewError.value = ''
  try {
    pipelineReviewOverview.value = await $fetch<PipelineReviewOverview>('/api/pipeline-review', {
      query: { project_root: project.value.source_root, session_id: selectedSession.value.session_id },
    })
  } catch (error) {
    pipelineReviewOverview.value = null
    const failure = error as { data?: { statusMessage?: string }; message?: string }
    pipelineReviewError.value = failure.data?.statusMessage ?? failure.message ?? '파이프라인 상세 검토를 준비하지 못했습니다.'
  } finally {
    pipelineReviewBusy.value = false
  }
}
async function openPipelineReview() {
  pipelineReviewOpen.value = true
  pipelineReviewOverview.value = null
  await refreshPipelineReview()
}
...
<template>
  <div class="hybrid-app">
    <header class="hybrid-header">
      ...
      <div class="view-toggle" aria-label="화면 보기"><button :class="{ active: viewMode === 'focus' }" @click="viewMode = 'focus'"><LayoutList :size="15" />상세 보기</button><button :class="{ active: viewMode === 'board' }" @click="viewMode = 'board'"><Columns3 :size="15" />전체 현황</button></div>
      <div class="header-tools"><label class="search-control"><Search :size="16" /><input v-model="searchQuery" placeholder="작업, 실행, 근거 검색" /></label><label class="project-select"><span class="sr-only">프로젝트 선택</span><select v-model="activeProjectRoot" @change="selectProjectRoot"><option v-for="item in data?.projects ?? []" :key="item.source_root" :value="item.source_root">{{ item.name }}</option></select></label><button class="icon-action" title="프로젝트 추가" @click="openProjectEditor"><FolderPlus :size="16" /></button><button class="launch-button" :disabled="source?.mode !== 'live' || !selectedSession" @click="openLaunchPanel"><Code2 :size="16" />작업 준비</button></div>
    </header>

    <section v-if="searchQuery.trim().length >= 2" class="search-results-panel" aria-label="검색 결과">
      <header><div><Search :size="15" /><strong>현재 프로젝트 검색</strong></div><span>{{ searchResults.length }}건</span></header>
      <button v-for="result in searchResults" :key="`${result.kind}-${result.session_id}-${result.run_id ?? ''}`" type="button" @click="chooseSearchResult(result)"><span>{{ result.kind === 'run' ? 'RUN' : 'SESSION' }}</span><div><strong>{{ result.title }}</strong><small>{{ result.detail }}</small></div><ChevronRight :size="15" /></button>
      <p v-if="!searchResults.length">일치하는 작업·실행·근거·산출물이 없습니다.</p>
    </section>

    <PipelineReviewPanel v-if="pipelineReviewOpen" :overview="pipelineReviewOverview" :loading="pipelineReviewBusy" :error="pipelineReviewError" @close="pipelineReviewOpen = false" @refresh="refreshPipelineReview" />
    <ExecutionTemplateDialog v-if="executionTemplateOpen && project?.source_root" :project-root="project.source_root" :expected-revision="project.relationship_revision ?? 0" :runs="allRuns" @close="executionTemplateOpen = false" @saved="acceptExecutionTemplate" />
    <RunAssetPanel v-if="assetPanelKind && selectedRun" :run="selectedRun" :kind="assetPanelKind" @close="assetPanelKind = null" />
    <RunReviewDialog v-if="reviewDialogOpen && selectedRun" :run="selectedRun" :saving="reviewSaving" :error="reviewError" @close="reviewDialogOpen = false" @save="saveRunReview" />
    ...
    <main class="hybrid-main">
      ...
      <section class="project-control-bar"><label><span>프로젝트</span><select v-model="activeProjectRoot" @change="selectProjectRoot"><option v-for="item in data?.projects ?? []" :key="item.source_root" :value="item.source_root">{{ item.name }}</option></select></label><button type="button" class="secondary-action" @click="openProjectEditor"><FolderPlus :size="15" />프로젝트 추가</button><button type="button" class="danger-action" title="프로젝트 제거 · 원본 폴더와 산출물은 보존" :disabled="activeCatalogProject?.origin !== 'catalog' || projectRemoving" @click="removeActiveProject"><Trash2 :size="15" /><span>프로젝트 제거</span></button><button type="button" class="secondary-action pipeline-review-action" title="선택한 작업의 템플릿·Run·근거·산출물·검증을 상세히 확인" :disabled="source?.mode !== 'live' || !selectedSession" @click="openPipelineReview"><FileCheck2 :size="15" />파이프라인 상세 검토</button></section>
      <section class="hero-row">
        ...
        <div class="summary-block"><button type="button" title="통과 결과를 보드에서 보기" :disabled="passCount === 0" @click="navigateSummary('pass')"><span class="ok"><Check :size="17" /></span><div><small>통과</small><strong>{{ passCount }}</strong></div></button><button type="button" title="검토가 필요한 실행 보기" :disabled="reviewCount === 0" @click="navigateSummary('review')"><span class="wait"><Clock3 :size="17" /></span><div><small>검토 필요</small><strong>{{ reviewCount }}</strong></div></button><button type="button" title="관계 충돌 확인하기" :disabled="(data?.conflicts.length ?? 0) === 0" @click="navigateSummary('conflict')"><span class="risk"><AlertTriangle :size="17" /></span><div><small>충돌</small><strong>{{ data?.conflicts.length ?? 0 }}</strong></div></button></div>
      </section>
      ...
      <div class="desktop-content">
        <template v-if="viewMode === 'focus'">
          ...
          <section class="focus-workspace">
            <aside class="session-panel"><div class="panel-label session-label"><span>작업 세션</span><div class="session-tools"><label title="작업 세션 정렬"><span class="sr-only">작업 세션 정렬</span><select :value="sessionSortMode" :disabled="source?.mode !== 'live' || sessionOrderSaving" @change="changeSessionSort"><option value="manual">수동 순서</option><option value="newest">최신순</option><option value="oldest">오래된순</option><option value="name">이름순</option></select></label><button type="button" title="작업 템플릿으로 생성" aria-label="작업 템플릿으로 생성" :disabled="source?.mode !== 'live' || !project" @click="executionTemplateOpen = true"><FileText :size="14" /></button><button type="button" title="빈 작업 세션 추가" aria-label="빈 작업 세션 추가" :disabled="source?.mode !== 'live'" @click="openSessionEditor"><Plus :size="14" /></button><button type="button" title="선택한 작업 세션 제거" aria-label="선택한 작업 세션 제거" :disabled="source?.mode !== 'live' || !selectedSession || sessionRemoving" @click="removeSelectedSession"><Minus :size="14" /></button></div></div><button v-for="session in visibleSessions" :key="session.session_id" :draggable="sessionSortMode === 'manual' && source?.mode === 'live'" :class="{ active: session.session_id === selectedSessionId, dragging: session.session_id === draggedSessionId }" :title="sessionSortMode === 'manual' ? '드래그하여 순서 변경 · 더블클릭하여 이름 편집 · 우클릭하여 제거' : '더블클릭하여 이름 편집 · 우클릭하여 제거'" @click="chooseSession(session)" @dblclick.stop="startSessionMetadataEdit(session)" @contextmenu.prevent="openSessionContextMenu($event, session)" @dragstart="startSessionDrag($event, session)" @dragover.prevent @drop.prevent="dropSession(session)" @dragend="finishSessionDrag"><GripVertical class="drag-handle" :size="13" /><span :class="session.relation_status" /><div><strong>{{ session.name }}</strong><small>{{ session.operation_kind === 'continue' ? '이어가기' : session.operation_kind === 'branch' ? '분기' : '새 작업' }} · {{ session.runs.length }}개 실행</small></div></button><p v-if="sessionOrderError" class="session-order-error">{{ sessionOrderError }}</p><div v-if="!sessions.length" class="empty-session"><strong>아직 작업이 없습니다.</strong><small>+ 버튼으로 빈 작업 세션을 추가하세요.</small></div><div class="readonly-note"><ShieldCheck :size="17" /><span><strong>{{ source?.mode === 'live' ? 'Engine 원본 보호됨' : 'Mock 원본 보호됨' }}</strong><small>{{ sessionSortMode === 'manual' ? '드래그 순서는 자동 저장됩니다.' : '자동 정렬 중 · 수동 순서는 보존됩니다.' }}</small></span></div></aside>
            ...
            <aside v-if="selectedRun" class="decision-panel"><div class="decision-head"><span>선택한 실행 · {{ displayStatusLabel(selectedRun) }}</span><button class="edit-trigger" title="표시 정보 편집" @click="startMetadataEdit"><Pencil :size="13" />편집</button></div><div class="identity-block" title="더블클릭하여 표시 정보 편집" @dblclick="startMetadataEdit(selectedRun)"><strong :title="selectedRun.run_id">{{ runDisplayName(selectedRun) }}</strong><small>원본 {{ runSystemLabel(selectedRun) }} · {{ runShortRef(selectedRun) }}</small><p v-if="selectedRun.user_note">{{ selectedRun.user_note }}</p><div v-if="selectedRun.tags?.length" class="metadata-tags"><span v-for="tag in selectedRun.tags" :key="tag">{{ tag }}</span></div></div><div class="decision-state" :class="selectedRun.status"><small>현재 판정</small><strong>{{ runLabel(selectedRun) }}</strong><p>{{ selectedRun.next_action }}</p></div><div class="evidence-health"><div><span>근거 충족도</span><strong>{{ selectedRun.evidence_count ? '충족' : '부족' }}</strong></div><div><i :style="{ width: selectedRun.evidence_count ? '82%' : '18%' }" /></div><small>근거 {{ selectedRun.evidence_count }}개 · 산출물 {{ selectedRun.artifact_count }}개</small></div><div class="asset-links"><button type="button" @click="openAssetPanel('evidence')"><FileCheck2 :size="16" /><span><strong>근거 목록</strong><small>{{ selectedRun.evidence_count }}개 연결</small></span><ChevronRight :size="14" /></button><button type="button" @click="openAssetPanel('artifact')"><FileText :size="16" /><span><strong>산출물 목록</strong><small>{{ selectedRun.artifact_count }}개 등록</small></span><ChevronRight :size="14" /></button></div><button type="button" class="review-action" @click="openReviewDialog()">{{ reviewLabel(selectedRun) }} <ArrowRight :size="15" /></button></aside>
          </section>
        </template>

        <section v-else class="refined-board">
          <aside v-if="selectedRun" class="board-selection-bar">
            <div>
              <small>현재 선택 · {{ selectedSession?.name }}</small>
              <strong>{{ runDisplayName(selectedRun) }}</strong>
              <p>{{ selectedRun.next_action }}</p>
            </div>
            <span :class="['board-selection-state', selectedRun.status]">{{ runLabel(selectedRun) }}</span>
            <div class="board-selection-actions">
              <button type="button" @click="openAssetPanel('evidence')"><FileCheck2 :size="14" />근거 {{ selectedRun.evidence_count }}</button>
              <button type="button" @click="openAssetPanel('artifact')"><FileText :size="14" />산출물 {{ selectedRun.artifact_count }}</button>
              <button type="button" @click="openReviewDialog()"><UserCheck :size="14" />{{ reviewLabel(selectedRun) }}</button>
              <button type="button" @click="startMetadataEdit"><Pencil :size="14" />편집</button>
              <button type="button" class="focus-return" @click="viewMode = 'focus'"><LayoutList :size="14" />상세 보기</button>
            </div>
          </aside>
          ...
          <div class="board-column relationship"><header><span><Link2 :size="16" /></span><strong>관계 확인</strong><em>{{ relationIssueCount }}</em></header><article v-if="relationIssueCount === 0"><small>RELATION</small><h3>관계 확인 완료</h3><p>현재 범위에서 누락되거나 충돌한 실행 관계가 없습니다.</p><footer><span class="pass"><Check :size="12" />confirmed</span></footer></article><button v-for="session in relationSessions" v-else :key="session.session_id" :class="{ selected: session.runs.some(run => run.run_id === selectedRun?.run_id) }" @click="chooseRun(session.runs[0])"><small>WORK SESSION</small><h3>{{ session.name }}</h3><p>{{ session.relation_status }}</p><footer><span class="hold">확인 필요</span></footer></button><article v-for="conflict in data?.conflicts" :key="conflict.relation_id" class="conflict"><small>RELATION</small><h3>{{ conflict.source_id }}</h3><p>{{ conflict.reason }}</p><footer><span>! conflict</span></footer></article></div>
          <div class="board-column execution featured"><header><span><Play :size="15" /></span><strong>실행·검증</strong><em>{{ attentionRuns.length }}</em></header><article v-if="attentionRuns.length === 0"><small>RUN</small><h3>추가 검토 없음</h3><p>현재 읽은 실행은 모두 통과 상태입니다.</p><footer><span class="pass"><Check :size="12" />clear</span></footer></article><button v-for="run in attentionRuns" v-else :key="run.run_id" :class="{ selected: run.run_id === selectedRun?.run_id }" @click="chooseRun(run)"><small>원본 {{ runSystemLabel(run) }} · {{ platformLabel(run.platform) }}</small><h3 :title="run.run_id">{{ runDisplayName(run) }}</h3><p>{{ run.next_action }}</p><footer><span class="evidence">{{ runLabel(run) }}</span><em>근거 {{ run.evidence_count }}</em></footer></button></div>
          <div class="board-column review"><header><span><UserCheck :size="16" /></span><strong>사용자 검토</strong><em>{{ pendingReviewRuns.length }}</em></header><button v-for="run in pendingReviewRuns" :key="run.run_id" :class="{ selected: run.run_id === selectedRun?.run_id }" @click="openReviewDialog(run)"><small>원본 {{ runSystemLabel(run) }} · {{ platformLabel(run.platform) }}</small><h3 :title="run.run_id">{{ runDisplayName(run) }}</h3><p>{{ run.review_note || run.next_action }}</p><div class="ready-box"><CheckCircle2 :size="17" /><span><strong>{{ reviewLabel(run) }}</strong><small>산출물 {{ run.artifact_count }} · 근거 {{ run.evidence_count }}</small></span></div><footer><span class="pass"><Check :size="12" />{{ reviewLabel(run) }}</span></footer></button></div>
        </section>
      </div>
      ...
      <div v-if="sessionContextMenu" class="session-context-menu" :style="{ left: `${sessionContextMenu.x}px`, top: `${sessionContextMenu.y}px` }" role="menu" @click.stop>
        <header><strong>{{ sessionContextMenu.session.name }}</strong><small>Run {{ sessionContextMenu.session.runs.length }}개</small></header>
        <button type="button" role="menuitem" @click="openTemplateFromContext"><FileText :size="14" />템플릿으로 생성</button>
        <button type="button" role="menuitem" @click="openEmptySessionFromContext"><Plus :size="14" />빈 작업 세션 추가</button>
        <button type="button" class="danger" role="menuitem" :disabled="sessionRemoving" @click="removeContextSession"><Trash2 :size="14" />작업 세션 제거</button>
        <small>Run과 산출물 원본은 삭제되지 않습니다.</small>
      </div>
      ...
      <section class="mobile-pipeline" aria-label="모바일 작업 흐름">
        ...
        <div class="mobile-stage review"><header><span><UserCheck :size="16" /></span><div><small>04</small><strong>사용자 검토</strong></div><em>{{ pendingReviewRuns.length }}건</em></header><button v-for="run in pendingReviewRuns" :key="run.run_id" @click="openReviewDialog(run)"><h2 :title="run.run_id">{{ runDisplayName(run) }}</h2><p>{{ run.review_note || run.next_action }}</p><span class="mobile-tag pass"><Check :size="12" />{{ reviewLabel(run) }}</span><small>원본 {{ runSystemLabel(run) }} · {{ runShortRef(run) }}</small><ChevronRight :size="16" /></button></div>
        <aside v-if="selectedRun" class="mobile-detail"><div><span class="run-status" :class="selectedRun.status">{{ selectedRun.status === 'pass' ? '✓' : selectedRun.status === 'hold' ? 'Ⅱ' : '!' }}</span><div><small>현재 선택 · {{ displayStatusLabel(selectedRun) }} · 원본 {{ runSystemLabel(selectedRun) }}</small><strong :title="selectedRun.run_id">{{ runDisplayName(selectedRun) }}</strong></div></div><p>{{ selectedRun.user_note || selectedRun.next_action }}</p><div class="mobile-detail-actions"><button type="button" @click="openAssetPanel('evidence')">근거 {{ selectedRun.evidence_count }}</button><button type="button" @click="openAssetPanel('artifact')">산출물 {{ selectedRun.artifact_count }}</button><button type="button" @click="openReviewDialog()">{{ reviewLabel(selectedRun) }}</button><button title="표시 정보 편집" @click="startMetadataEdit"><Pencil :size="14" />편집</button></div></aside>
      </section>
    </main>
  </div>
</template>
...
.session-tools{display:contents}.session-tools label{grid-column:1/-1;grid-row:2;min-width:0;display:block}.session-tools button:nth-of-type(1){grid-column:2;grid-row:1}.session-tools button:nth-of-type(2){grid-column:3;grid-row:1}.session-tools button:nth-of-type(3){grid-column:4;grid-row:1}.session-tools select{width:100%;height:27px;padding:0 20px 0 6px;border:1px solid #cbd7d2;background:#fff;color:#53625d;border-radius:4px;font-size:8px}.session-panel>button{padding:8px 6px;grid-template-columns:13px 8px minmax(0,1fr);gap:6px}.session-panel>button[draggable="true"]{cursor:grab}.session-panel>button[draggable="true"]:active{cursor:grabbing}.session-panel>button.dragging{opacity:.48;border-color:#77a28f;background:#edf6f2}.drag-handle{color:#9ba7a2}.session-panel>button:not([draggable="true"]) .drag-handle{opacity:.25}.session-order-error{margin:5px 5px 0;padding:6px 7px;color:#9d403a;background:#fff1ef;border-radius:4px;font-size:8px}.readonly-note span{min-width:0}.readonly-note small{white-space:normal}
.search-results-panel{position:fixed;top:58px;right:24px;z-index:90;width:min(440px,calc(100vw - 48px));max-height:420px;overflow:auto;padding:8px;background:#fff;border:1px solid #cfd8d5;border-radius:6px;box-shadow:0 16px 44px #10251d2b}.search-results-panel>header{height:34px;padding:0 4px;display:flex;align-items:center;justify-content:space-between}.search-results-panel>header div{display:flex;align-items:center;gap:6px}.search-results-panel>header strong{font-size:9px}.search-results-panel>header span{color:#75817d;font-size:8px}.search-results-panel>button{width:100%;min-height:48px;padding:8px;display:grid;grid-template-columns:52px 1fr 16px;align-items:center;gap:8px;text-align:left;border:0;border-top:1px solid #e4e9e7;background:#fff;color:inherit}.search-results-panel>button:hover{background:#f3f8f5}.search-results-panel>button>span{color:#2f6c56;font-size:7px;font-weight:800}.search-results-panel>button div{display:grid;gap:3px}.search-results-panel>button strong{font-size:9px}.search-results-panel>button small{color:#75817d;font-size:8px}.search-results-panel>p{margin:0;padding:22px;text-align:center;color:#75817d;font-size:9px}.mobile-detail-actions{display:grid!important;grid-template-columns:1fr 1fr;gap:6px!important}.mobile-detail-actions button{margin:0!important;justify-content:center!important}
.session-context-menu{position:fixed;z-index:150;width:210px;padding:8px;background:#fff;border:1px solid #cad5d1;border-radius:6px;box-shadow:0 14px 38px #10251d30}.session-context-menu header{padding:3px 4px 8px;display:grid;gap:3px;border-bottom:1px solid #e2e8e6}.session-context-menu header strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:9px}.session-context-menu header small,.session-context-menu>small{color:#75817d;font-size:7px}.session-context-menu button{width:100%;height:32px;margin:4px 0;display:flex;align-items:center;gap:6px;border:0;background:#f3f7f5;color:#356957;border-radius:4px;font-size:9px;font-weight:800}.session-context-menu button:hover{background:#e8f2ed}.session-context-menu button.danger{margin-top:7px;background:#fff1ef;color:#9d403a}.session-context-menu button.danger:hover{background:#fbe4e1}.session-context-menu button:disabled{opacity:.5}
@media(max-width:700px){.search-results-panel{top:62px;right:12px;width:calc(100vw - 24px)}}
</style>
