...
여기서 **AI 운영**은 거창한 서버 관리가 아닙니다. AI에게 작업을 맡긴 뒤 아래 내용을 다시 확인할 수 있게 관리하는 과정입니다.

1. 내가 무엇을 요청했는지
2. AI가 어떤 작업을 실행했는지
3. 결과를 뒷받침하는 근거가 있는지
4. 실제 산출물 파일이 만들어졌는지
5. 내가 결과를 확인했고 다음에 무엇을 해야 하는지

이 대시보드는 위 다섯 가지를 프로젝트별로 정리합니다. Python이나 데이터 스키마를 몰라도 화면에서 **통과·근거 부족·보류**와 다음 행동을 먼저 확인할 수 있습니다.

### 이런 상황에 적합합니다

- 여러 AI CLI를 사용하면서 결과 폴더가 흩어지는 경우
- 같은 프로젝트를 며칠 뒤 다른 터미널에서 이어서 작업하는 경우
- AI의 완료 보고만으로 실제 파일과 검증 결과를 판단하기 어려운 경우
- 새 작업, 이어가기와 분기를 구분해 기록하고 싶은 경우
- 자동화는 사용하되 최종 판단은 사람이 유지하고 싶은 경우

단순한 일회성 질문이나 결과 기록이 필요 없는 짧은 대화에는 이 도구를 사용할 필요가 없습니다.
...
처음부터 실제 프로젝트를 연결하기보다 익명 Mock 데이터로 화면을 먼저 확인하는 방식을 권장합니다.

### 1단계: 화면 둘러보기

아래의 [빠른 화면 확인](#빠른-화면-확인)을 실행한 뒤 `http://localhost:3215/hybrid`를 엽니다. Mock 데이터는 실제 사용자 파일을 읽거나 수정하지 않습니다.

화면에서는 다음 세 가지만 먼저 확인하면 됩니다.

- **프로젝트**: AI 작업을 모아 두는 최상위 폴더
- **작업 세션**: 사용자가 이해하는 하나의 작업 주제
- **실행 기록(Run)**: AI와 Engine이 실제로 작업한 한 번의 기록

### 2단계: 첫 결과 읽기

실행 기록 하나를 선택하고 다음 순서로 확인합니다.

1. 상태가 통과·근거 부족·보류 중 무엇인지 확인합니다.
2. 근거 목록에서 판단을 뒷받침하는 파일과 설명을 확인합니다.
3. 산출물 목록에서 실제 결과 파일을 확인합니다.
4. 파이프라인 상세 검토에서 요청과 완료 조건이 연결됐는지 확인합니다.
5. 충분히 확인했다면 사용자 검토 완료로 표시합니다.

### 3단계: 실제 프로젝트 연결

테스트용 폴더를 하나 준비하고 대시보드에 ProjectRoot로 등록합니다. 작업 템플릿에서 현재 상황과 제약을 입력한 뒤, 사용할 CLI의 준비 명령을 확인합니다. 실제 CLI 실행은 Codex·Claude Code·Antigravity의 터미널에서 진행되며 대시보드는 생성된 Run을 읽어 작업 세션에 연결합니다.

상세한 첫 실행 절차와 문제 대응은 [AI 운영 입문 가이드](deliverables/getting_started_for_ai_operations.md)에 정리했습니다.
...
> **중요:** 통과는 “현재 자동 검증 기준을 충족했다”는 뜻입니다. 결과가 마음에 든다는 뜻이나 최종 승인과는 다릅니다. 근거와 산출물을 확인한 뒤 사용자가 별도로 판단해야 합니다.
...
Dashboard `1.0.6` 동기화에서는 운영판 추적 파일 90개와 공개 저장소를 SHA-256으로 비교해 `90/90` 일치를 확인했습니다. 현재 README와 문서 인덱스의 내부 링크 누락은 없으며, 공개 범위에서 사용자 절대경로와 비밀값 신규 노출도 발견되지 않았습니다.
...
| 문서 | 내용 |
|---|---|
| [AI 운영 입문 가이드](deliverables/getting_started_for_ai_operations.md) | 설치 전 이해할 개념, 첫 작업, 상태 확인과 문제 대응 |
